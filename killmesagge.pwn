...
public OnPlayerDeath(playerid, killerid, reason) //В калбек який викликається після смерті персонажа
{
	if(killerid != INVALID_PLAYER_ID) //перевірка на валідність, щоб не спрацьовувало якщо гравець вбив сам себе.
	{
  	    ...
  	    format(string, sizeof(string), "Ви були вбиті гравцем {F81414}%s[%d]. {FFFFFF}За допомогою: %s", PI[killerid][pNames], killerid, killreason[reason]); //форматуємо сам текст, PI[killerid][pNames] - змінити на змінну яка зберігає в собі нік, з аргументом killerid
