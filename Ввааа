...
#define SEARCH_CORPSE	2700 //тривалість першого лута трупа (в секундах)
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) //паблік роботи із діалогами
{
	case D_DEAD_AMMO:
	{
		...
		SetPVarInt(playerid, "pListGun", listitem); //В пвар записю ID вибраної із списка зброї
...
stock CreateDeathActor(playerid) //створення трупа
{
	...
	for (new i = 0; i <= 12; i++) //перевірка всіх слотів на зброю
	{
